...
const SCHEMAS = {
    anonymous: [
        ...
        { key: 'amount', label: 'Amount', type: 'text', required: true, placeholder: 'e.g., 100 tablets' },
        { key: 'batch_lot_number', label: 'Batch / Lot Number', type: 'text', required: false },
        { key: 'date_of_dispensing', label: 'Date of Dispensing', type: 'date', required: false },
        { key: 'reaction_category', label: 'Reaction Category', type: 'text', required: false },
        { key: 'severity', label: 'Severity', type: 'select', options: ['mild', 'moderate', 'severe'], required: false, help: 'Select the severity level of the reaction' },
        ...
        { key: 'age_group', label: 'Age Group', type: 'select', options: ['pediatric', 'adolescent', 'adult', 'elderly', 'unknown'], required: false },
    ...
    identified: [
        { key: 'patient_name', label: 'Patient Name', type: 'text', required: true, placeholder: 'Full name' },
        { key: 'patient_email', label: 'Patient Email', type: 'text', required: false, placeholder: 'patient@email.com (if available)' },
        { key: 'patient_phone', label: 'Patient Phone (WhatsApp)', type: 'text', required: false, placeholder: '+91XXXXXXXXXX (if available)' },
        { key: 'drug_name', label: 'Drug Name', type: 'text', required: true, placeholder: 'e.g., Aspirin 500mg' },
        { key: 'amount', label: 'Amount', type: 'text', required: true, placeholder: 'e.g., 100 tablets' },
        { key: 'reaction_category', label: 'Disease/Reaction', type: 'text', required: true, placeholder: 'e.g., Headache, Nausea' },
        { key: 'severity', label: 'Severity', type: 'select', options: ['mild', 'moderate', 'severe'], required: false },
        { key: 'additional_notes', label: 'Additional Notes', type: 'textarea', required: false }
    ],
    aggregated: [
        { key: 'drug_name', label: 'Drug Name', type: 'text', required: true, placeholder: 'e.g., Aspirin' },
        { key: 'amount', label: 'Total Amount Dispensed', type: 'number', required: true, placeholder: 'e.g., 1000' },
        { key: 'total_reactions_reported', label: 'Total Reactions Reported', type: 'number', required: false },
        { key: 'mild_count', label: 'Mild Count', type: 'number', required: false },
        { key: 'moderate_count', label: 'Moderate Count', type: 'number', required: false },
        { key: 'severe_count', label: 'Severe Count', type: 'number', required: false },
        { key: 'reporting_period_start', label: 'Reporting Period Start', type: 'date', required: false },
        { key: 'reporting_period_end', label: 'Reporting Period End', type: 'date', required: false },
    ...
    reader.onload = function (e) {
        try {
            ...
            if (missingColumns.length > 0 || extraColumns.length > 0) {
                ...
                // Convert Excel labels to schema keys for backend compatibility
                const labelToKeyMap = {};
                schema.forEach(f => {
                    labelToKeyMap[f.label] = f.key;
                });
                ...
                reportState.excelData = convertedRows;
...
function validateFormFields() {
    ...
    rows.forEach((row, rowIdx) => {
        ...
        // For identified reports, check if at least one contact method is provided
        if (reportState.reportType === 'identified') {
            const emailInput = row.querySelector('input[name="patient_email"]');
            const phoneInput = row.querySelector('input[name="patient_phone"]');
            const hasEmail = emailInput && emailInput.value.trim() !== '';
            const hasPhone = phoneInput && phoneInput.value.trim() !== '';

            if (!hasEmail && !hasPhone) {
                errors.push(`Record ${rowIdx + 1}: At least Email or Phone is required for notifications`);
                // Highlight both fields
                if (emailInput) {
                    const emailGroup = emailInput.closest('.form-group');
                    if (emailGroup) {
                        emailGroup.classList.add('error');
                        const errorSpan = emailGroup.querySelector('.field-error');
                        if (errorSpan) errorSpan.textContent = 'Email or Phone required';
                    }
                ...
                if (phoneInput) {
                    ...
                    if (phoneGroup) {
                        ...
                        const errorSpan = phoneGroup.querySelector('.field-error');
                        if (errorSpan) errorSpan.textContent = 'Email or Phone required';
                    }
                }
            }
        ...
        inputs.forEach(input => {
            ...
            } else if (!formGroup.classList.contains('error')) {
    ...
    fetch('/api/pharmacy/reports/submit', {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // Show patient creation info
                if (data.patients_created && data.patients_created > 0) {
                    message += ` | ${data.patients_created} patient(s) created for PV tracking`;
                }
                
                // Show notification info
                if (data.followup_results && data.followup_results.length > 0) {
                    const sentCount = data.followup_results.filter(f => f.status === 'sent').length;
                    if (sentCount > 0) {
                        message += ` | Notifications sent for ${sentCount} patient(s)`;
                    }
                }
                ...
                console.log('Submission details:', {
                    ...
                    patients_created: data.patients_created,
